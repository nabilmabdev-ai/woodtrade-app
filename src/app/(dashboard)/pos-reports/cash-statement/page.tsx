        ...
        <div className="printable-area">
              ...
              <tbody className="bg-white divide-y divide-gray-200">
                {reportData.transactions.map((tx, index) => (
                  <tr key={index}>
                    ...
                    <td className={`px-4 py-3 whitespace-nowrap text-right font-semibold ${tx.amount >= 0 ? 'text-green-700' : 'text-red-700'}`}>{tx.amount.toFixed(2)} €</td>
                  </tr>
                ))}
              </tbody>
            </table>
          ...
          <div className="p-6 border rounded-lg bg-white shadow-sm max-w-md ml-auto">
            ...
            <div className="space-y-2 text-sm">
                ...
                <hr className="my-2"/>
                <div className="flex justify-between"><span>+ Fonds de caisse initiaux (sessions)</span><span className="font-medium">{reportData.summary.totalOpeningFundsInPeriod.toFixed(2)} €</span></div>
                {Object.entries(reportData.summary.salesByMethod).map(([method, total]) => (
                    <div key={method} className="flex justify-between pl-4"><span>+ Ventes ({method})</span><span className="font-medium">{total.toFixed(2)} €</span></div>
                ))}
                {Object.entries(reportData.summary.refundsByMethod).map(([method, total]) => (
                    <div key={method} className="flex justify-between pl-4"><span>- Remboursements ({method})</span><span className="font-medium text-red-600">{total.toFixed(2)} €</span></div>
                ))}
                <div className="flex justify-between"><span>+ Apports (Pay-in)</span><span className="font-medium">{reportData.summary.totalPayIns.toFixed(2)} €</span></div>
                <div className="flex justify-between"><span>- Dépenses (Pay-out)</span><span className="font-medium text-red-600">{reportData.summary.totalPayOuts.toFixed(2)} €</span></div>
                <div className="flex justify-between"><span>- Retraits (Dépôt, etc.)</span><span className="font-medium text-red-600">{reportData.summary.totalWithdrawals.toFixed(2)} €</span></div>
                <div className="flex justify-between"><span>+ Transferts Reçus</span><span className="font-medium text-purple-600">{reportData.summary.totalTransfersIn.toFixed(2)} €</span></div>
                <div className="flex justify-between"><span>- Transferts Envoyés</span><span className="font-medium text-purple-600">-{reportData.summary.totalTransfersOut.toFixed(2)} €</span></div>
                
                <hr className="my-2"/>
                <div className="flex justify-between font-bold"><span>= Flux de caisse net</span><span>{reportData.summary.netCashFlow.toFixed(2)} €</span></div>
