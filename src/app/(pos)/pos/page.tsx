...
export default function PosPage() {
  ...
  useEffect(() => { const fetchRegisters = async () => { try { const res = await fetch('/api/cash-registers'); if (!res.ok) throw new Error("Impossible de charger les caisses."); const allRegisters: CashRegister[] = await res.json(); const salesRegisters = allRegisters.filter(r => r.type === CashRegisterType.SALES); setCashRegisters(salesRegisters); } catch(e) { toast.error((e as Error).message); } finally { setInitialLoading(false); } }; fetchRegisters(); }, []);
  ...
  // --- ✅ ROBUST QUANTITY HANDLER APPLIED HERE ---
  const handleUpdateQuantity = (variantId: string, newQuantity: number) => {
    // If the new quantity is not a number (e.g., from an empty input), treat it as 0.
  ...
  if (authLoading || initialLoading) return <div className="min-h-screen bg-gray-800 flex items-center justify-center"><p className="text-white text-2xl">Chargement...</p></div>;
  if (!selectedRegister) return ( <div className="min-h-screen bg-gray-800 flex flex-col items-center justify-center p-4"><h1 className="text-4xl font-bold text-white mb-8">WoodTrade POS</h1><div className="w-full max-w-md bg-white rounded-lg shadow-xl p-8"><h2 className="text-2xl font-semibold mb-6 text-gray-700 text-center">1. Choisir une caisse de Vente</h2><div className="space-y-4">{cashRegisters.map(register => (<button key={register.id} onClick={() => handleSelectRegister(register)} className="w-full text-left p-4 border rounded-lg text-lg font-semibold hover:bg-blue-50 hover:border-blue-400 transition-all">{register.name}</button>))
      }
      {
          cashRegisters.length === 0 && (<p className="text-center text-gray-500 py-4">Aucune caisse de type &quot;Vente&quot; n&apos;a été configurée.</p>)
      }
      </div></div></div> );
  if (isCheckingSession) return <div className="min-h-screen bg-gray-800 flex items-center justify-center"><p className="text-white text-2xl">Vérification de la session...</p></div>;
  if (!activeSession) return ( <div className="min-h-screen bg-gray-800 flex flex-col items-center justify-center p-4 text-center"><h1 className="text-4xl font-bold text-white mb-8">Session Requise</h1><div className="w-full max-w-md bg-white rounded-lg shadow-xl p-8"><h2 className="text-2xl font-semibold mb-4 text-red-600">Aucune session ouverte !</h2><p className="text-gray-600 mb-6">Pour commencer à vendre sur &quot;{selectedRegister.name}&quot;, une session doit être ouverte.</p><Link href={`/cash-registers/${selectedRegister.id}`} className="block w-full text-center p-4 bg-blue-600 text-white rounded-lg font-semibold hover:bg-blue-700">Aller à la Gestion de Caisse</Link><button onClick={() => setSelectedRegister(null)} className="mt-4 text-sm text-gray-500 hover:underline">Retour</button></div></div> );
  if (!selectedCustomer) return ( <> <div className="min-h-screen bg-gray-800 flex flex-col items-center justify-center p-4"><h1 className="text-4xl font-bold text-white mb-4">WoodTrade POS</h1><p className="text-lg text-green-300 mb-4">Caisse: {selectedRegister.name} (Session active)</p><div className="w-full max-w-2xl bg-white rounded-lg shadow-xl p-8"><h2 className="text-2xl font-semibold mb-4 text-gray-700">2. Sélectionner un client (F4)</h2><div className="flex space-x-4 mb-4"><input ref={customerSearchRef} type="text" placeholder="Rechercher un client..." className="w-full p-4 border rounded-lg text-lg" value={customerSearchTerm} onChange={(e) => setCustomerSearchTerm(e.target.value)} /><button onClick={() => setIsCreateCustomerModalOpen(true)} className="px-6 py-2 font-semibold text-white bg-green-600 rounded-md hover:bg-green-700 whitespace-nowrap">+ Nouveau</button></div><div className="h-64 overflow-y-auto">{isSearchingCustomers ? <p className="text-center text-gray-500">Recherche...</p> : <ul className="divide-y divide-gray-200">{customerSearchResults.map(customer => (<li key={customer.id} onClick={() => handleSelectCustomer(customer)} className="p-4 hover:bg-blue-50 cursor-pointer"><p className="font-semibold">{customer.name}</p></li>))}</ul>}</div></div></div>
        {isCreateCustomerModalOpen && (<div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50"><div className="bg-white p-8 rounded-lg shadow-2xl w-full max-w-lg"><h2 className="text-2xl font-bold mb-6">Créer un nouveau client</h2><form onSubmit={handleCreateCustomer} className="space-y-4"><fieldset className="p-4 border rounded-md"><legend className="text-lg font-semibold px-2">Entreprise</legend><div className="grid grid-cols-1 md:grid-cols-2 gap-4"><div><label className="block text-sm">Nom *</label><input type="text" name="name" value={newCustomerData.name} onChange={handleNewCustomerChange} required className="w-full p-2 border rounded-md"/></div><div><label className="block text-sm">N° TVA</label><input type="text" name="vat" value={newCustomerData.vat} onChange={handleNewCustomerChange} className="w-full p-2 border rounded-md"/></div></div></fieldset><fieldset className="p-4 border rounded-md"><legend className="text-lg font-semibold px-2">Contact Principal</legend><div className="grid grid-cols-1 md:grid-cols-2 gap-4"><div><label className="block text-sm">Prénom *</label><input type="text" name="firstName" value={newCustomerData.firstName} onChange={handleNewCustomerChange} required className="w-full p-2 border rounded-md"/></div><div><label className="block text-sm">Nom *</label><input type="text" name="lastName" value={newCustomerData.lastName} onChange={handleNewCustomerChange} required className="w-full p-2 border rounded-md"/></div><div><label className="block text-sm">Email</label><input type="email" name="email" value={newCustomerData.email} onChange={handleNewCustomerChange} className="w-full p-2 border rounded-md"/></div><div><label className="block text-sm">Téléphone</label><input type="text" name="phone" value={newCustomerData.phone} onChange={handleNewCustomerChange} className="w-full p-2 border rounded-md"/></div></div></fieldset><div className="flex justify-end space-x-4 pt-4"><button type="button" onClick={() => setIsCreateCustomerModalOpen(false)} className="px-4 py-2 bg-gray-100 rounded-md hover:bg-gray-200">Annuler</button><button type="submit" className="px-4 py-2 text-white bg-blue-600 rounded-md hover:bg-blue-700">Créer</button></div></form></div></div>)}
      </>
    );
    ...
    <>
      <div className="flex h-screen bg-gray-100 font-sans">
        <div className="w-3/5 flex flex-col p-4">
          <header className="mb-4 flex justify-between items-center"><div><h1 className="text-2xl font-bold text-gray-800">WoodTrade POS</h1><p className="text-sm text-gray-500">Caisse: {selectedRegister.name}</p></div><div className="text-right"><p className="font-semibold text-lg text-gray-700">{selectedCustomer.name}</p><button onClick={handleChangeCustomer} className="text-sm text-blue-600 hover:underline">(Changer)</button></div></header>
          <div className="mb-4"><label htmlFor="product-search" className="sr-only">Rechercher (F2)</label><input id="product-search" ref={productSearchRef} type="text" placeholder="Rechercher un produit (F2)..." className="w-full p-4 border rounded-lg text-lg" value={productSearchTerm} onChange={(e) => setProductSearchTerm(e.target.value)} /></div>
          <div className="flex-1 bg-white p-4 rounded-lg shadow-inner overflow-y-auto"><div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">{productSearchResults.map((product) => (<div key={product.id} onClick={() => handleAddToCart(product)} className="border rounded-lg p-4 text-center cursor-pointer hover:bg-blue-50 hover:shadow-md transition-all"><h3 className="font-semibold h-12">{product.name}</h3><p className="text-gray-500 text-sm">{product.sku}</p><p className="text-lg font-bold mt-2">{(product.variants[0]?.prices[0]?.price || 0).toFixed(2)} {CURRENCY_LABEL}</p></div>))}</div></div>
        </div>
        
        <div className="w-2/5 flex flex-col bg-white p-6 shadow-lg">
          <div className="flex justify-between items-center mb-4"><h2 className="text-xl font-bold">Vente</h2><button onClick={handleClearCart} className="text-sm text-red-500 hover:underline">Vider</button></div>
          <div className="flex-1 overflow-y-auto border-t border-b py-4 space-y-3">{cart.length === 0 ? (<p className="text-center text-gray-500">Panier vide.</p>) : (cart.map(item => (<div key={item.variantId} className="p-2 rounded-md hover:bg-gray-50"><div className="flex justify-between items-center"><div className="flex-grow pr-2"><p className="font-semibold leading-tight">{item.productName}</p><p className="text-sm text-gray-500">{item.quantity} x {item.unitPrice.toFixed(2)} {CURRENCY_LABEL}</p></div><div className="flex items-center space-x-2">
            
            {/* --- ✅ UI FIX APPLIED HERE: Replaced span with a number input --- */}
            ...
            <input
                ...
                className="w-14 text-center font-semibold border rounded-md p-1"
            />
            <button onClick={() => handleUpdateQuantity(item.variantId, item.quantity + 1)} className="w-7 h-7 rounded-full bg-gray-200 hover:bg-gray-300">+</button>
            
            </div><div className="font-bold text-lg w-24 text-right">{(item.unitPrice * item.quantity).toFixed(2)} {CURRENCY_LABEL}</div><div className="flex items-center space-x-2 ml-2"><button onClick={() => openDiscountModal(item)} className="text-gray-400 hover:text-blue-500"><TagIcon /></button><button onClick={() => handleUpdateQuantity(item.variantId, 0)} className="text-gray-400 hover:text-red-500"><TrashIcon /></button></div></div>{item.discount > 0 && (<div className="text-right text-sm text-red-600">- {item.discount.toFixed(2)} {CURRENCY_LABEL} (remise)</div>)}</div>)))}</div>
          <div className="mt-auto pt-6">
            <div className="space-y-2 mb-4 text-lg"><div className="flex justify-between"><span>Sous-total</span><span>{subtotal.toFixed(2)} {CURRENCY_LABEL}</span></div><div className="flex justify-between text-red-600"><span>Remise</span><span>- {totalDiscount.toFixed(2)} {CURRENCY_LABEL}</span></div><div className="flex justify-between items-center text-2xl font-bold border-t pt-2 mt-2"><span>TOTAL</span><span>{grandTotal.toFixed(2)} {CURRENCY_LABEL}</span></div></div>
            <div className="bg-gray-50 p-4 rounded-lg">
              <div className="flex justify-between font-bold text-xl mb-4"><span>Restant Dû</span><span className={amountDue > 0 ? 'text-red-600' : 'text-green-600'}>{amountDue.toFixed(2)} {CURRENCY_LABEL}</span></div>
              {payments.map((p, index) => (<div key={index} className="flex justify-between items-center bg-white p-2 rounded-md mb-2"><span className="font-semibold">{p.method}</span><span>{p.amount.toFixed(2)} {CURRENCY_LABEL}</span><button onClick={() => removePayment(index)} className="text-red-500 hover:text-red-700 font-bold">X</button></div>))}
