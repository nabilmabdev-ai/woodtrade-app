        ...
        <div className="w-2/5 flex flex-col bg-white p-6 shadow-lg">
          ...
          <div className="mt-auto pt-6">
            ...
            <div className="bg-gray-50 p-4 rounded-lg">
              <div className="flex justify-between font-bold text-xl mb-4"><span>Restant Dû</span><span className={amountDue > 0 ? 'text-red-600' : 'text-green-600'}>{amountDue.toFixed(2)} €</span></div>
              {payments.map((p, index) => (<div key={index} className="flex justify-between items-center bg-white p-2 rounded-md mb-2"><span className="font-semibold">{p.method}</span><span>{p.amount.toFixed(2)} €</span><button onClick={() => removePayment(index)} className="text-red-500 hover:text-red-700 font-bold">X</button></div>))}
