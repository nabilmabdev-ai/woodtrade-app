// src/app/(dashboard)/suppliers/new/page.tsx
...
export default function NewSupplierPage() {
  ...
  // State for both supplier and its primary contact
  ...
  const handleSubmit = async (event: FormEvent) => {
    ...
    const promise = fetch('/api/suppliers', {
      method: 'POST',
      ...
      if (!response.ok) {
        ...
        return Promise.reject(errorData.error || 'Impossible de créer le fournisseur');
      }
      return response.json();
    });

    toast.promise(promise, {
      loading: 'Ajout du fournisseur...',
      success: () => {
        // On successful creation, redirect back to the suppliers list
      ...
      <div className="max-w-4xl mx-auto p-8 border rounded-lg bg-white shadow-md">
        <form onSubmit={handleSubmit} className="space-y-6">
          ...
          <fieldset className="space-y-4">
            <legend className="text-lg font-semibold text-gray-700 border-b pb-2 mb-4">Informations sur le fournisseur</legend>
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
              <div>
                <label htmlFor="name" className="block text-sm font-medium text-gray-700">Nom du fournisseur *</label>
              ...
              <div>
                ...
                <input id="category" type="text" value={category} onChange={(e) => setCategory(e.target.value)} placeholder="Ex: Bois, Quincaillerie" className="mt-1 block w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm focus:ring-blue-500 focus:border-blue-500"/>
              </div>
            </div>
          </fieldset>
