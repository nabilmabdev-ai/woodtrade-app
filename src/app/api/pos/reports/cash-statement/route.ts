...
export async function GET(request: Request) {
  ...
  try {
    ...
    // The logic for fetching data remains the same...
    let openingBalance = 0;
    if (caisseIds && caisseIds.length > 0) {
      const lastClosedSession = await prisma.cashRegisterSession.findFirst({
        where: { status: 'CLOSED', closedAt: { lt: start }, cashRegister: { id: { in: caisseIds }, type: CashRegisterType.SALES } },
        orderBy: { closedAt: 'desc' },
      });
      openingBalance = lastClosedSession?.closingBalance ?? 0;
    }
    
    const sessionsOpenedInPeriod = await prisma.cashRegisterSession.findMany({ where: { openedAt: { gte: start, lte: end }, ...baseWhereClause, }, include: { cashRegister: true, openedByUser: true } });
    const payments = await prisma.payment.findMany({ where: { paymentDate: { gte: start, lte: end }, invoiceId: { not: null }, cashRegisterSession: { ...baseWhereClause }, }, include: { invoice: { include: { order: { include: { company: true, user: true } } } }, cashRegisterSession: { include: { cashRegister: true } } } });
    const refunds = await prisma.refund.findMany({ where: { refundDate: { gte: start, lte: end }, cashRegisterSession: { ...baseWhereClause } }, include: { returnOrder: { include: { originalOrder: { include: { company: true, user: true } } } }, cashRegisterSession: { include: { cashRegister: true } } } });
    const movements = await prisma.cashMovement.findMany({ where: { createdAt: { gte: start, lte: end }, session: { ...baseWhereClause } }, include: { user: true, session: { include: { cashRegister: true } } } });
    ...
    const transactions = [
      ...
      // Utilisation de l'accès optionnel `?.` pour éviter un crash si `m.session` est nul.
    ...
    // The summary calculation logic remains the same...
